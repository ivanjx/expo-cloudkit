...
8. **Durable asset lease.** Download and persist the returned owner/version/URI/byte count. Force-quit/relaunch and use **Verify persisted downloads and upload still readable**. Require byte counts to match and source uploads to remain present. Native staged files are caller-owned on successful delivery; the lab records them, never deletes upload sources, and does not claim checksum/identity acceptance on your application's behalf.
